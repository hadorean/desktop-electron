<script lang="ts">
    ...
    $: {
        if (
            ...
            !images.some((img) => img.name === $settings.selectedImage)
        ) {
...
<div class="settings-panel" bind:this={settingsPanel} class:expanded={expanded}>
    <div class="settings-content">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-semibold">Settings</h2>
            <div class="flex items-center space-x-2">
                <button
                    class="btn btn-sm {!$isLocalMode
                        ? 'btn-primary'
                        : 'btn-ghost'}"
            ...
            <div class="setting-section">
                ...
                <ImageGrid
                    ...
                    onImageChange={(newImage) =>
                        handleSettingChange("selectedImage", newImage)}
                />

                <SliderControl
                    label="Brightness"
                    value={$isLocalMode
                        ? ($localSettings?.opacity ?? null)
                        : $sharedSettings.opacity}
                    min={0}
                    max={1}
                    step={0.01}
                    onChange={(newOpacity) =>
                        handleSettingChange("opacity", newOpacity)}
                ...
                <SliderControl
                    label="Saturation"
                    value={$isLocalMode
                        ? ($localSettings?.saturation ?? null)
                        : $sharedSettings.saturation}
                    min={0}
                    max={2}
                    step={0.01}
