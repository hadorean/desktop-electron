<script lang="ts">
    ...
    const effectiveImage = $derived(
        isOverridden ? overrideValue : selectedImage,
    );
</script>
...
<div class="form-control w-full">
    ...
    <div
        class="grid grid-cols-3 gap-2 bg-base-200 bg-opacity-30 p-2 rounded-lg overflow-y-auto max-h-[280px]"
        class:ghost={isOverride && !isOverridden}
    >
        {#each sortedImages as image}
            <div
                class="thumbnail-container {effectiveImage === image.name
                    ? 'selected'
                    : ''}"
                ...
                onkeydown={(e) =>
                    e.key === "Enter" && handleImageClick(image.name)}
                tabindex="0"
                role="button"
