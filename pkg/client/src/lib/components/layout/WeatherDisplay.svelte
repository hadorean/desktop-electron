...
{#if weather}
  <div
    class="fixed inset-x-0 top-1/2 flex flex-col items-center justify-center pointer-events-none"
    style="opacity: {opacity.current};"
  >
    <div
      class="text-white text-4xl font-light opacity-80 mb-2 text-center text-shadow-lg"
    >
      {greeting}
    </div>
    <div
      class="text-white text-2xl font-light opacity-80 mb-4 text-center text-shadow-lg"
    >
    ...
    <div
      class="text-white text-4xl opacity-80 flex items-center justify-center text-shadow-lg"
    >
      <span class="mr-2">
        {#if weather.current.condition
          .toLowerCase()
          .includes("sunny") || weather.current.condition
            .toLowerCase()
            .includes("clear")}
        ...
        {:else if weather.current.condition
          .toLowerCase()
          .includes("rain") || weather.current.condition
            .toLowerCase()
            .includes("shower")}
          🌧️
        ...
        {:else if weather.current.condition
          .toLowerCase()
          .includes("storm") || weather.current.condition
            .toLowerCase()
            .includes("thunder")}
          ⛈️
        {:else if weather.current.condition
          .toLowerCase()
          .includes("fog") || weather.current.condition
            .toLowerCase()
            .includes("mist")}
          🌫️
        {:else}
